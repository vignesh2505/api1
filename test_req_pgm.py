...
def test_post_request(token):
    url = "https://sustainium-dev.12thwonder.com/api/app/purchased-gas-management"
    payload = {
...
def test_delete_request(token, resource_id):
    url = f"https://sustainium-dev.12thwonder.com/api/app/purchased-gas-management/{resource_id}"
    headers = {
        "tenantIdentifier": TENANT_IDENTIFIER,
        "Authorization": f"Bearer {token}"
    }
