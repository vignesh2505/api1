...
def test_post_request(token):
    url = "https://sustainium-dev.12thwonder.com/api/app/downstream-management"
    payload = {
...
def test_delete_request(token, resource_id):
    url = f"https://sustainium-dev.12thwonder.com/api/app/downstream-management/{resource_id}"
    headers = {
        "tenantIdentifier": TENANT_IDENTIFIER,
        "Authorization": f"Bearer {token}"
    }
