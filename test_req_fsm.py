...
def test_post_request(token):
    url = "https://sustainium-dev.12thwonder.com/api/app/fire-suppression-management"
    payload = {
...
def test_delete_request(token, resource_id):
    url = f"https://sustainium-dev.12thwonder.com/api/app/fire-suppression-management/{resource_id}"
    headers = {
        "tenantIdentifier": TENANT_IDENTIFIER,
        "Authorization": f"Bearer {token}"
    }
