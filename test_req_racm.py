...
def test_post_request(token):
    url = "https://sustainium-dev.12thwonder.com/api/app/refrigeration-aCs-management"
    payload = {
    ...
    "monthsData": {
        "2025-2025": {
            "newUnitCharge": 20,
            "newUnitCapacity": 100,
            "existingUnitRecharge": 30,
            "disposedUnitCapacity": 40,
            "disposedUnitRecovered": 50
        }
    },
    "description": "test1",
    "emissionSourceId": "3a1287cd-e0ef-9639-c836-64636da4d961",
...
def test_delete_request(token, resource_id):
    url = f"https://sustainium-dev.12thwonder.com/api/app/refrigeration-aCs-management/{resource_id}"
    headers = {
        "tenantIdentifier": TENANT_IDENTIFIER,
        "Authorization": f"Bearer {token}"
    }
